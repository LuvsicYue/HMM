...
for i in range(len(index)):
    ...
    A.append(map(lambda x:(x,''), nltk.word_tokenize(shake[index[i]+2][0])))
    A.append(map(lambda x:(x,''), nltk.word_tokenize(shake[index[i]+3][0])))
    A.append(map(lambda x:(x,''), nltk.word_tokenize(shake[index[i]+4][0])))
    A.append(map(lambda x:(x,''), nltk.word_tokenize(shake[index[i]+5][0])))
    A.append(map(lambda x:(x,''), nltk.word_tokenize(shake[index[i]+6][0])))
    A.append(map(lambda x:(x,''), nltk.word_tokenize(shake[index[i]+7][0])))
    A.append(map(lambda x:(x,''), nltk.word_tokenize(shake[index[i]+8][0])))
#print len(A)
